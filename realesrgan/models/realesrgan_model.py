...
from basicsr.losses.custom_loss import LteLoss, LeeLoss
...
class RealESRGANModel(SRGANModel):
    def __init__(self, opt):
        ...
        self.lte_loss, self.lee_loss = None, None


        if train_opt.get('lte_opt'):
            self.lte_loss = build_loss(train_opt['lte_opt']).to(self.device)
        if train_opt.get('lee_opt'):
            self.lee_loss = build_loss(train_opt['lee_opt']).to(self.device)

        extra_params = []
        if isinstance(self.lte_loss, LteLoss):
            extra_params += list(self.lte_loss.lte.parameters())
        if isinstance(self.lee_loss, LeeLoss):
            extra_params += list(self.lee_loss.edge_extractor.parameters())
        if extra_params:
            self.optimizer_g.add_param_group({'params': extra_params})

    @torch.no_grad()
    def _dequeue_and_enqueue(self):
        b, c, h, w = self.lq.size()
        if not hasattr(self, 'queue_lr'):
            assert self.queue_size % b == 0, f'queue size {self.queue_size} phải chia hết cho batch size {b}'
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            # LTE Loss
            if self.lte_loss:
                l_g_lte = self.lte_loss(self.output, self.gt)
                l_g_total += l_g_lte
                loss_dict['l_g_lte'] = l_g_lte

            # LEE Loss
            if self.lee_loss:
                l_g_lee = self.lee_loss(self.output, self.gt)
                l_g_total += l_g_lee
                loss_dict['l_g_lee'] = l_g_lee
            ...
            # update EMA teacher cho loss
            if isinstance(self.lte_loss, LteLoss):
                self.lte_loss.update_teacher()
            if isinstance(self.lee_loss, LeeLoss):
                self.lee_loss.update_teacher()
