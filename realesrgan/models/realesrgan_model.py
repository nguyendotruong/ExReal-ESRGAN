...
from basicsr.losses.custom_loss import *   # EdgeLoss, LteLoss
from basicsr.losses import build_loss      # để build các loss từ opt

@MODEL_REGISTRY.register()
class RealESRGANModel(SRGANModel):
    """RealESRGAN Model cho Real-ESRGAN với EdgeLoss và LTE Loss."""
    ...
    def __init__(self, opt):
        ...
        # Khởi tạo EdgeLoss và LteLoss nếu có trong train opt
        train_opt = self.opt.get('train')
        self.edge_loss = None
        self.lte_loss = None
        if train_opt.get('edge_opt'):
            self.edge_loss = build_loss(train_opt['edge_opt']).to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            # EdgeLoss
            if self.edge_loss:
                l_g_edge = self.edge_loss(self.output, self.gt)
                l_g_total += l_g_edge
