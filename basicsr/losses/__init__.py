...
from .custom_loss import LteLoss, LeeLoss
...
def build_loss(opt):
    ...
    if 'lte' in opt and isinstance(opt['lte'], dict):
        opt['lte'] = build_network(opt['lte'])
        
    if 'edge_extractor' in opt and isinstance(opt['edge_extractor'], dict):
        opt['edge_extractor'] = build_network(opt['edge_extractor'])
